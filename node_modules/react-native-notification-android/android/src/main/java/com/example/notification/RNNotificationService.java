...
public class RNNotificationService {
    ...
    private static final String ACTION_DELETE_BROADCAST ="com.dhms.broadcast.notification.delete";
    ...
    public void onCreate() {
        mContext.registerReceiver(mDeleteReceiver,new IntentFilter(ACTION_DELETE_BROADCAST));
    }

    public void onBind(Service service) {
        mNotificationId = calcAppId();
        Notification notification=new Notification.Builder(mContext.getApplicationContext()).build();
    ...
    public void onDestroy() {
        mNotificationNum = 0;
        mInstance = null;
        mContext.unregisterReceiver(mDeleteReceiver);
    }
    ...
    private void doShowNotification(String message) {
        Log.d(TAG, "showNotification:"+message +" num:"+mNotificationNum);
...
//        intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK|Intent.FLAG_ACTIVITY_RESET_TASK_IF_NEEDED);
            ...
            intent.setClassName(mContext.getPackageName(), getMainActivityClass());
            notification = new Notification.Builder(mContext)
                    ...
                    .setDeleteIntent(PendingIntent.getBroadcast(mContext ,0 , new Intent(ACTION_DELETE_BROADCAST) ,0))
                    .build();
        } else {
            notification = new Notification.Builder(mContext)
                    .setSmallIcon(appIcon)
    ...
    private String getMainActivityClass() {
        ...
        for (ActivityManager.RunningTaskInfo task : runningTasks) {
            if (task.baseActivity.getPackageName().equalsIgnoreCase(packageName)) {
                String appClass = task.baseActivity.getClassName();
                Log.d(TAG,"MainActivityClass:"+appClass);
                return appClass;
            }
        }
        return null;
    ...
    private BroadcastReceiver mDeleteReceiver =new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (ACTION_DELETE_BROADCAST.equals(action)){
                Log.d(TAG, "Receive broadcast:"+ACTION_DELETE_BROADCAST);
                deleteNotification();
            }
        }
    };
